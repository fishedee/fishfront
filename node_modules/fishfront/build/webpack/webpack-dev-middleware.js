...
module.exports = function (compiler, options) {
	...
	if (options.lazy) {
		if (typeof options.filename === "string") {
			var str = options.filename.replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g, "\\$&").replace(/\\\[[a-z]+\\\]/ig, ".+");
	...
	compiler.plugin("done", function (stats) {
		...
		process.nextTick(function () {
			...
			if (!options.noInfo && !options.quiet) console.info("webpack: bundle is now VALID.");
	...
	function invalidPlugin() {
		if (state && !options.noInfo && !options.quiet) console.info("webpack: bundle is now INVALID.");
	...
	function ready(fn, req) {
		if (state) return fn();
		if (!options.noInfo && !options.quiet) console.log("webpack: wait until bundle finished: " + req.url);
		callbacks.push(fn);
	}
	...
	function webpackDevMiddleware(req, res, next) {
		...
		if (options.lazy && (!options.filename || options.filename.test(filename))) rebuild();
	...
	webpackDevMiddleware.close = function (callback) {
		...
		if (watching) watching.close(callback);else callback();
	};
