...
exports.default = {
	...
	replaceTagStyle: function replaceTagStyle(content, tag, style) {

		if (!content) {
			return content;
		}

		var tagRegex = new RegExp('<' + tag + '(.*?)>', 'ig');
		var styleRegex = /style=\"(.*?)\"/ig;
		var tagReplace = '<' + tag + ' $1/>';
		var tagReplace2 = '<' + tag + ' $1 style="' + style + '"/>';
		...
		content = content.replace(tagRegex, function (match) {
			if (match.match(styleRegex)) {
				...
				match = match.replace(tagRegex, tagReplace2);
			}
			return match;
		});
		return content;
