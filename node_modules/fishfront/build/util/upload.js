...
exports.default = {
	_checkFileSize: function _checkFileSize(file, defaultOption, index, nextStep) {
	...
	_checkFileSelect: function _checkFileSelect(file, defaultOption, index, multiCount, nextStep) {
		if (file.files) {
			//支持HTML5的浏览器
	...
	_checkFileType: function _checkFileType(file, defaultOption, index, nextStep) {
		//测试后缀来检查文件类型
	...
	_readImage: function _readImage(file, defaultOption, index, nextStep) {
	...
	_compressImage: function _compressImage(file, defaultOption, index, nextStep) {
		//配置压缩图片的选项
		var conf = {
	...
	_cloudImageUpload: function _cloudImageUpload(file, defaultOption, index, nextStep) {
	...
	_localImageUpload: function _localImageUpload(file, defaultOption, index, nextStep) {
		//构造数据
		...
		httpReuqest.open("POST", defaultOption.url + argConnect + 't=' + Date.now(), true);
		httpReuqest.withCredentials = true;
		httpReuqest.addEventListener('progress', progress, false);
		httpReuqest.addEventListener("load", complete, false);
		httpReuqest.addEventListener("abort", abort, false);
		httpReuqest.addEventListener("error", failed, false);
	...
	_uploadImage: function _uploadImage(file, defaultOption, index, nextStep) {
		var self = this;
	...
	_uploadImageList: function _uploadImageList(index, file, defaultOption, multiCount) {
   ...
   */
	...
	imageV3: function imageV3(option) {
		...
		var defaultOption = {
			...
			quality: 0.8,
			multi: false,
			getMultiCount: function getMultiCount() {
				return 1;
			},
