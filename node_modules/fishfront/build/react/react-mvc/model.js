...
function combileFunctor(methodName, prevFuctor, nextFunctor) {
	var methodNames = {
		"initialize": true
	};
	if (!prevFuctor) {
		...
		if (!methodNames.hasOwnProperty(methodName)) {
			console.warn("multi model method " + methodName);
			return prevFuctor;
		} else {
			return function () {
				nextFunctor.apply(this, arguments);
				prevFuctor.apply(this, arguments);
			};
		}
	}
}
function combineMixin(proto) {
	if (!proto.mixins) {
		return;
	}
	...
	try {
		for (var _iterator = proto.mixins[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
			...
			for (var methodName in singleMixin) {
				...
				proto[methodName] = combileFunctor(methodName, proto[methodName], methodResult);
			}
		}
	} catch (err) {
...
function createClass(proto) {
	...
	proto.loadModel = function (modelClass) {
		var modelInstanse = this.__context.create(modelClass);
		this[modelInstanse.name] = modelInstanse;
	};
	function StoreClass(context) {
		var _this = this;
